    ...
    <div class="container-fluid">
     @if (session('error'))
        ...
        <div class="row">
            <div class="col-12">
                {{-- Filter --}}
                <form style="margin: 20px 0" method="GET" action="{{ route('backend.posts.index') }}"
                    class="form-inline">
                    <div class="col-3">
                        <input value="{{ request()->get('title') }}" name="title" type="text" class="form-control"
                            placeholder="Nhập tiêu đề cần tìm..">
                    </div>
                    <div class="col-3">
                        <input value="{{ request()->get('status') }}" name="status" type="text" class="form-control"
                ...
                <div class="card">
                    <div class="card-header">
                        @can('create-post', App\Models\Post::class)
                            <a href="{{ route('backend.posts.create') }}" class="btn btn-success"><i style="margin-right:10px"
                                    class="fas fa-plus"></i>Tạo bài viết</a>
                        @endcan
                            ...
                            <tbody>
                                @foreach ($posts as $key => $post)
                                    <tr>
                                        ...
                                        <td>
                                            @if (!empty($post->image))
                                                <img src="{{ $post->image_url_full }}" width="80px" height="50px">
                                            @endif
                                        </td>
